...
class ModelModuleExchangeWmd extends Model {
    ...
    public function setCategory($data){
        ...
        if(!isset($query->row['category_id'])){
            ...
            foreach ($query->rows as $language) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$language['language_id'] . "', name = '" . $this->db->escape($data['name']) . "', description = '', meta_title = '" . $this->db->escape($data['name']) . "', meta_description = '', meta_keyword = ''");
            }
    ...
    public function updateProduct($product_id, $data){
        ...
        if(isset($data['description'])){
            foreach ($query_language->rows as $language) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language['language_id'] . "', name = '" . $this->db->escape($data['description']['name']) . "', description = '" . $this->db->escape($data['description']['description']) . "', meta_title = '" . $this->db->escape($data['description']['name']) . "'");
            }
        }
    ...
    public function addManufacturer($name){
        ...
        if($this->config->get('exchange_wmd_manufacturer_description')){
            $query_language = $this->db->query("SELECT language_id FROM " . DB_PREFIX . "language");
            foreach ($query_language->rows as $language) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer_description SET manufacturer_id = '" . (int)$manufacturer_id . "', language_id = '" . (int)$language['language_id'] . "', name = '" . $this->db->escape(trim($name)) . "', meta_title = '" . $this->db->escape(trim($name)) . "'");
            }
        }
