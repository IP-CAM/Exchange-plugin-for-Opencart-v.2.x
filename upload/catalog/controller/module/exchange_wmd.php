        ...
        if(isset($result['data'])){
            ...
            foreach($result['data'] as $product){
                ...
                if((int)$product['deleted']){
                    ...
                    $markup = $product['price'] * ((float)$this->config->get('exchange_wmd_markup_size') / 100);
                    ...
                    if($product['image']){
                        ...
                        $img_path = 'catalog/wmd_import/' . $product_id . "_" . $image_name;
        ...
        if(isset($result['data'])){
            foreach($result['data'] as $order){
                ...
                if(isset($exchange_wmd_order_status[$status]) && $store_orders[$id_order]['order_status_id'] != $exchange_wmd_order_status[$status]){
                    $this->model_checkout_order->addOrderHistory($store_orders[$id_order]['order_id'], $exchange_wmd_order_status[$status], '', (int)$this->config->get('exchange_wmd_notify_order_status'));
                }
            }
        }
    }
